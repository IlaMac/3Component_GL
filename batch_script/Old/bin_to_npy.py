...
for l in range(len(L)):
    ...
    for b in range(nbeta):
        ...
        ###############################
        #  Remove the old .bin files  #
        ###############################

        os.remove(file_DS + ".bin")
        os.remove(file_M + ".bin")
        os.remove(file_E + ".bin")
        os.remove(file_Psi + ".bin")
...
for l in range(len(L)):
    ...
    for b in range(nbeta):
        ...
        # E M DS Psi1 Psi2 Psi3 -> 6 columns
        Out=np.reshape(Out, (len(Out)/6,6))
        E=Out[:,0]
        M=Out[:,1]
        Ds=Out[:,2]
        Psi1=Out[:,3]
        Psi2=Out[:,4]
        Psi3=Out[:,5]
        Psi=np.column_stack((Psi1, Psi2, Psi3))
